...
- 支持非法字符检测：非法字符不超过 10%（包含 10%），音频会正常生成并返回非法字符占比；非法字符超过 10%，接口不返回结果（返回报错码），请检测后再次进行请求

> **非法字符定义**：ASCII 码中的控制符（不含制表符 `\t` 和换行符 `\n`）
...
1. 若使用文件输入，需先调用 **文件上传 API** 获取 `file_id`
2. 调用 **创建语音生成任务 API** 创建任务
3. 调用 **查询语音生成任务状态 API** 查询进度
4. 当任务完成时，使用返回的 `file_id` 调用 **文件下载 API** 下载音频

> ⚠️ **注意**：返回的下载 URL 自生成起 **9 小时（32400 秒）**内有效，过期后文件将失效，请及时下载。
...
"""
本示例用于创建语音合成任务。
若使用文件作为输入，则需要将 <text_file_id> 替换为文本文件的 file_id。
若使用文本作为输入，则设置 "text" 字段。
注意：需要先将密钥信息写入环境变量 `MINIMAX_API_KEY`。
"""
import requests
import json
...
```bash
curl --location 'https://api.minimaxi.com/v1/t2a_v2_async' \
--header 'Authorization: Bearer YOUR_API_KEY' \
...
api_key = os.environ.get("MINIMAX_API_KEY")
url = f"https://api.minimaxi.com/v1/t2a_v2_async/query?task_id={task_id}"
...
```bash
curl --location "https://api.minimaxi.com/v1/t2a_v2_async/query?task_id=YOUR_TASK_ID" \
--header 'Authorization: Bearer YOUR_API_KEY' \
--header 'Content-Type: application/json'
```
...
```bash
curl --location "https://api.minimaxi.com/v1/files/retrieve?file_id=YOUR_FILE_ID" \
--header 'Content-Type: application/json' \
--header 'Authorization: Bearer YOUR_API_KEY' \
--output "output.mp3"
```
...
- [异步语音合成](https://platform.minimaxi.com/docs) - 使用 API 接口，创建异步语音合成任务
