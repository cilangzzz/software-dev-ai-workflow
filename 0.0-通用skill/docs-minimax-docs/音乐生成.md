...
- [Music 3.0：即刻体验全新音乐生成能力](#music-30即刻体验全新音乐生成能力)
- [音乐生成示例](#音乐生成示例)
- [翻唱生成](#翻唱生成)
- [推荐阅读](#推荐阅读)
...
拿到歌词后，通过 `prompt` 设定音乐风格（如 "Big Band Swing, Brass Section, Walking Bass"），将歌词传入 `lyrics` 参数，音乐生成接口会为你编曲、演唱，输出一首完整的歌曲。如果没有歌词，可将 `lyrics_optimizer` 字段设置为 `true` 让模型自动优化歌词。
...
查看并编辑上一步返回的 `formatted_lyrics`，然后将 `cover_feature_id` 和修改后的歌词传入音乐生成接口。
...
- [音乐生成接口](https://platform.minimaxi.com/docs) - 使用 API 接口，输入歌词和歌曲描述，进行歌曲生成
- [翻唱前处理接口](https://platform.minimaxi.com/docs) - 对参考音频进行预处理，提取特征和歌词，用于两步翻唱生成
- [歌词生成接口](https://platform.minimaxi.com/docs) - 使用 API 接口，输入歌曲描述，进行歌词生成或编辑
